...
def run(params=params):
    ...
    contact = 0
    time=0.
    while i<steps-1 and Z[i]>=-hpore/2.-2.:
        xi_x=gauss(0.,1.)
        xi_y=gauss(0.,1.)
        xi_z=gauss(0.,1.)
        Force = F
        ...
        Z[i+1] = Z[i] + coeff*xi_z*math.sqrt(Dfac) + C*Force[2]*Dfac + fp(Z[i])*Dmol*tau
        time+=.05
        if dis(argument(X[i+1],Y[i+1],Z[i+1])) < rMolecule:
            ...
            if np.random.binomial(1,P_bind)==1 and Z[i]<=hpore/2.-h2:
                bind+=1
    ...
    T=[list(T)]
    J1=[list(J1)]
    fields.save_fields("randomwalk1",params,X=X,Y=Y,Z=Z,T=T,J=J1)
#    fields.update()
...
#    if tau_off<.2e6:
#        np.save('TAU_1',np.append(np.load('TAU_1.npy'),tau_off))
#        np.save('A_1',np.append(np.load('A_1.npy'),amplitude))
#        print 'tau_off = %.3f ms'% (tau_off*1e-6)
#        print 'Type I'
#    else:
#        np.save('TAU_2',np.append(np.load('TAU_2.npy'),tau_off))
#        np.save('A_2',np.append(np.load('A_2.npy'),amplitude))
#        print 'tau_off = %.3f ms'% (tau_off*1e-6)
#        print 'Type II'
#    print 'molecule as trans? ',Z[i]<-hpore/2.-1.
#    if plotres:
