...
def uPB(x):
    return 0. #eps*E0/eta*(phi(x) - phi(R))
def r(x):
...
J_PB_u = 0. #assemble(Constant(cFarad*c0*E0/lscale**2)*J_u*r2pi*dx)
J_PB = J_PB_el + J_PB_u
print("J (PB): %s [A]" % J_PB)
...
def J(U, geo):
    ...
    Jsurf = avg(Constant(cFarad/lscale**2)*(Jp - Jm)[2] * r2pi) * geo.dS("cross")
    Jvol = Constant(cFarad/lscale**2/hcross)*(Jp - Jm)[2] * r2pi * geo.dx("cross")
    return dict(Jsurf=Jsurf, Jvol=Jvol)
"""
def saveJ(self):
...
problem = CoupledProblem(problems, couplers, geo3D, phys, cyl=False, conservative=False, ku=2) #, beta=10.)
...
#plot1D({"uz PNP (2D)":u[1]}, (0., R, 101), "x", dim=3, axlabels=("r [nm]", "velocity [m/s]"), newfig=False)
